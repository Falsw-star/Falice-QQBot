...
def opt(msg,name,content="",t="0",block=True):#msg 回复讯问人员 ma回复内容 content字符串，添加要存储的内容  name 字符串，存储名称 位置/save/name 有去重功能 c 1,添加，2，删除，3，列举
    ...
    if  t == "1":        
        ...
        with open(path, "a+") as f:
            ...
            if block :
                if found:#添加成功
                    message_create(msg['cid'],f"<at id='{msg['user']['id']}'/>添加失败喵")
                else :
                    message_create(msg['cid'],f"<at id='{msg['user']['id']}'/>添加成功喵")
    elif t == "2":
        new = ""
        Found = False
        ...
        with open(path, "w") as f :
            f.writelines(new);  
        if  Found   :
            message_create(msg['cid'],f"<at id='{msg['user']['id']}'/>删除成功喵")
        else:
            message_create(msg['cid'],f"<at id='{msg['user']['id']}'/>删除失败喵")
    else   : 
        try:
            with open(path, "r") as f:
                ...
                if line :
                    ...
                    message_create(msg['cid'],f"<at id='{msg['user']['id']}'/>喵~没有哦")
        except:
            message_create(msg['cid'],f"<at id='{msg['user']['id']}'/>喵~没有哦")
    return None